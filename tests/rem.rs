...
fn d64_checked_and_try_rem() {
    ...
    assert_eq!(D64::ONE.try_rem(D64::ZERO), Err(DecimalError::DivisionByZero));
    assert_eq!(D64::from_i32(10).checked_rem(D64::from_i32(3)), Some(D64::ONE));
...
fn d64_is_multiple_of_and_div_rem() {
    ...
    let (q, r) = D64::from_str("10.5").unwrap().div_rem(D64::from_i32(3)).unwrap();
    assert_eq!(q, 3);
...
fn d96_checked_and_try_rem() {
    ...
    assert_eq!(D96::ONE.try_rem(D96::ZERO), Err(DecimalError::DivisionByZero));
    assert_eq!(D96::from_i32(10).checked_rem(D96::from_i32(3)), Some(D96::ONE));
}
...
fn d96_is_multiple_of_and_div_rem() {
    ...
    let (q, r) = D96::from_str("10.5").unwrap().div_rem(D96::from_i32(3)).unwrap();
    assert_eq!(q, 3);
    assert_eq!(r.to_string(), "1.5");
}
