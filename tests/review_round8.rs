//! Regression tests for parsing and display precision hardening.
//!
...
fn d64_format_precision_pads_above_native_scale() {
    ...
    assert_eq!(
        format!("{:.10}", D64::from_str("1.5").unwrap()),
        "1.5000000000"
    );
    assert_eq!(
        format!("{:.12}", D64::from_str("1.5").unwrap()),
        "1.500000000000"
    );
    ...
    assert_eq!(
        format!("{:.12}", D64::from_str("-0.25").unwrap()),
        "-0.250000000000"
    );
...
fn d64_format_precision_below_native_scale_unchanged() {
    ...
    assert_eq!(
        format!("{:.4}", D64::from_str("3.14159").unwrap()),
        "3.1416"
    );
}
...
fn d96_format_precision_pads_above_native_scale() {
    ...
    assert_eq!(
        format!("{:.15}", D96::from_str("1.5").unwrap()),
        "1.500000000000000"
    );
    assert_eq!(
        format!("{:.15}", D96::from_str("-0.25").unwrap()),
        "-0.250000000000000"
    );
    assert_eq!(
        format!("{:.15}", D96::from_str("1.5").unwrap()),
...
fn d96_format_precision_below_native_scale_unchanged() {
    ...
    assert_eq!(
        format!("{:.6}", D96::from_str("3.14159265").unwrap()),
        "3.141593"
    );
}
...
fn d64_from_str_lossy_accepts_scientific() {
    ...
    assert_eq!(
        D64::from_str_lossy("1.5e2").unwrap(),
        D64::from_str("150").unwrap()
    );
    assert_eq!(
        D64::from_str_lossy("1E-3").unwrap(),
        D64::from_str("0.001").unwrap()
    );
    assert_eq!(
        D64::from_str_lossy("-2.5e-1").unwrap(),
    ...
    assert_eq!(
        D64::from_str_exact("1e-13"),
        Err(fixdec::DecimalError::PrecisionLoss)
    );
    ...
    assert_eq!(
        D64::from_str_lossy("1.234567895e0").unwrap(),
        D64::from_str("1.2345679").unwrap()
    );
    assert_eq!(
        D64::from_str_lossy("1.234567885e0").unwrap(),
        D64::from_str("1.23456788").unwrap()
    );
...
fn d96_from_str_lossy_accepts_scientific() {
    assert_eq!(
        D96::from_str_lossy("1.5e2").unwrap(),
        D96::from_str("150").unwrap()
    );
    assert_eq!(
        D96::from_str_lossy("1E-6").unwrap(),
        D96::from_str("0.000001").unwrap()
    );
    assert_eq!(
        D96::from_str_lossy("-2.5e-1").unwrap(),
        D96::from_str("-0.25").unwrap()
    );
    // Below the 12-dp scale: lossy rounds to zero rather than erroring.
    assert_eq!(
        D96::from_str_exact("1e-15"),
        Err(fixdec::DecimalError::PrecisionLoss)
    );
