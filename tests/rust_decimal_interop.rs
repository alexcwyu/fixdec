...
fn decimal_to_d64_exact_within_8dp() {
    assert_eq!(
        D64::from_rust_decimal(dec("123.45")).unwrap(),
        D64::from_str("123.45").unwrap()
    );
    assert_eq!(
        D64::from_rust_decimal(dec("0.00000001")).unwrap(),
        D64::from_str("0.00000001").unwrap()
    );
    // trailing zeros below 8dp still divide evenly -> exact
    assert_eq!(
        D64::from_rust_decimal(dec("1.230000000")).unwrap(),
        D64::from_str("1.23").unwrap()
    );
    assert_eq!(D64::from_rust_decimal(dec("0")).unwrap(), D64::ZERO);
    assert_eq!(
        D64::from_rust_decimal(dec("-7.5")).unwrap(),
        D64::from_str("-7.5").unwrap()
    );
}
...
fn decimal_to_d64_precision_loss_is_strict() {
    ...
    assert_eq!(
        D64::from_rust_decimal(dec("0.000000001")),
        Err(DecimalError::PrecisionLoss)
    );
    assert_eq!(
        D64::from_rust_decimal(dec("-1.234567891")),
        Err(DecimalError::PrecisionLoss)
    );
}
...
fn decimal_to_d64_overflow_and_underflow() {
    ...
    assert_eq!(
        D64::from_rust_decimal(dec("100000000000")),
    ...
    assert_eq!(
        D64::from_rust_decimal(Decimal::MAX),
        Err(DecimalError::Overflow)
    );
}
...
fn decimal_to_d64_round_bankers() {
    ...
    assert_eq!(
        D64::from_rust_decimal_round(dec("0.000000005")).unwrap(),
        D64::ZERO
    ); // 0.5 -> 0
    assert_eq!(
    ...
    assert_eq!(
        D64::from_rust_decimal_round(dec("0.0000000000001")).unwrap(),
        D64::ZERO
    );
    // rounding variant still rejects out-of-range magnitudes
    assert_eq!(
        D64::from_rust_decimal_round(dec("100000000000")),
        Err(DecimalError::Overflow)
    );
    assert_eq!(
        D64::from_rust_decimal_round(dec("-100000000000")),
...
fn decimal_to_d96_exact_and_precision_loss() {
    assert_eq!(
        D96::from_rust_decimal(dec("123.456789012")).unwrap(),
        D96::from_str("123.456789012").unwrap()
    );
    assert_eq!(
        D96::from_rust_decimal(dec("0.000000000001")).unwrap(),
        D96::from_str("0.000000000001").unwrap()
    );
    // 13th decimal significant -> PrecisionLoss
    assert_eq!(
        D96::from_rust_decimal(dec("0.0000000000001")),
        Err(DecimalError::PrecisionLoss)
    );
}
...
fn decimal_large_negative_is_underflow_not_overflow() {
    ...
    assert_eq!(
        D96::from_rust_decimal(Decimal::MIN),
        Err(DecimalError::Underflow)
    );
    assert_eq!(
        D96::from_rust_decimal_round(Decimal::MIN),
        Err(DecimalError::Underflow)
    );
    // The positive extreme remains Overflow; sign-specific errors are preserved.
    assert_eq!(
        D96::from_rust_decimal(Decimal::MAX),
        Err(DecimalError::Overflow)
    );
    assert_eq!(
        D96::from_rust_decimal_round(Decimal::MAX),
        Err(DecimalError::Overflow)
    );
    ...
    assert_eq!(
        D96::from_rust_decimal(dec("-100000000000000000")),
        Err(DecimalError::Underflow)
    );
    // D64 mirror (its overflow path is reached via the range check).
    assert_eq!(
        D64::from_rust_decimal(Decimal::MIN),
        Err(DecimalError::Underflow)
    );
    assert_eq!(
        D64::from_rust_decimal_round(Decimal::MIN),
        Err(DecimalError::Underflow)
    );
}
...
fn decimal_to_d96_round_and_overflow() {
    ...
    assert_eq!(
        D96::from_rust_decimal_round(dec("0.0000000000005")).unwrap(),
        D96::ZERO
    ); // 0.5 -> 0
    assert_eq!(
...
fn trait_and_inherent_forms_agree() {
    ...
    assert_eq!(
        <D64 as TryFrom<Decimal>>::try_from(v).unwrap(),
        D64::from_rust_decimal(v).unwrap()
    );

    let d96 = D96::from_str("2.718281828459").unwrap();
    let v96: Decimal = d96.into();
    assert_eq!(
        <D96 as TryFrom<Decimal>>::try_from(v96).unwrap(),
        D96::from_rust_decimal(v96).unwrap()
    );
}
