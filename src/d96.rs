...
impl D96 {
    ...
    /// `(100, 0)`, and `0.000000000001` to `(1, 12)`. `mantissa()` is `0` exactly
    /// when the value is zero, and always fits the 96-bit range (even at `MIN`,
    /// which is not divisible by 10 and so survives trimming unchanged).
    ///
    ...
    /// and `0.000000000001` has scale `12`. Pairs with [`mantissa`](Self::mantissa).
    #[inline]
    #[must_use]
    ...
    /// Returns `self` unchanged — D96 values are always normalized.
    ///
    /// `D96` is `repr(transparent)` over a single `i128` at a fixed scale, so each
    ...
    ///
    /// # Panics
    /// Panics if the value would overflow the 96-bit range, or if `fractional`
    /// is outside `[0, SCALE)`.
    pub const fn new(integer: i128, fractional: i128) -> Self {
        ...
        if fractional < 0 || fractional >= Self::SCALE {
            panic!("D96::new: fractional part must be in [0, SCALE)");
        }
    ...
    pub fn with_scale(mantissa: i128, scale: u32) -> Self {
        ...
        if scale_diff == 0 {
            ...
            let multiplier = pow10_i128(scale_diff as u8);
...
impl D96 {
    ...
    pub const fn round(self) -> Self {
        ...
        let rounded_quotient = banker_round_i128(quotient, remainder, half);
    ...
    pub const fn round_dp_with_strategy(
        ...
        let rounded = apply_rounding(q, r, rounding_factor, strategy);
        Self {
            value: Self::clamp_to_96(rounded * rounding_factor),
        }
    }
    ...
    pub const fn checked_quantize(self, tick: Self, strategy: RoundingStrategy) -> Option<Self> {
        ...
        match n.checked_mul(tick.value) {
    ...
    ///
    /// # Panics
    /// Panics if `tick <= 0` or the result overflows the 96-bit range.
    #[must_use = "this returns the result of the operation, without modifying the original"]
...
impl D96 {
    ...
    pub const fn to_d64_round(self) -> crate::Result<D64> {
        ...
        let rounded = banker_round_i128(quotient, remainder, 5_000);
...
impl D96 {
    ...
    pub const fn to_i128_round(self) -> i128 {
        ...
        banker_round_i128(quotient, remainder, half)
    }
...
impl D96 {
    ...
    pub fn from_str_exact(s: &str) -> crate::Result<Self> {
        ...
        let fractional_value = if let Some(dp_idx) = decimal_idx {
            ...
            (frac_digits as u128) * pow10_u128(remaining_decimals as u8)
        } else {
            0
        };
    ...
    pub fn from_fixed_point_str(s: &str, decimals: u8) -> crate::Result<Self> {
        ...
        let multiplier = pow10_i128(scale_diff);
...
impl D96 {
    ...
    fn fmt_with_precision(&self, f: &mut fmt::Formatter<'_>, precision: usize) -> fmt::Result {
        ...
        let divisor = pow10_u128(scale_down as u8);
        ...
        let precision_scale = pow10_u128(precision_capped as u8);
